...
I am a self-taught software engineer. I do not have a STEM degree, and I have not come up through a big-name company, so I wanted an easy way for people to see practical examples of what I can do.

## The experiment: AI first

This time, I wanted to build a site with AI doing as much of the work as possible. The idea was to push AI to do everything, find ways of working with it that maximise productivity without sacrificing quality, and document the process.
...
I felt inclined to trust the decision in this case, because I had heard of Astro before and knew it to be a well liked framework.
...
The official course might teach me the same material as the AI, but I trust it more, because the maintainers built it to teach the framework correctly and I trust it covers what they consider the core skills. With AI you can never quite be sure. If something is a core part of your stack, I still feel it is worth learning it from a reputable source rather than using it blindly.
...
I did make an early mistake. I got too excited and tried to do too many things at once, and the codebase turned into a mess. I then spent a long time refactoring it back into a usable state. The lesson: start from a solid base and try one thing at a time. Commit the designs you are happy with as you go, so the AI can change things freely without overwriting anything you wanted to keep.
...
There was still a lot of fiddly refactoring, and some designs proved so hard to turn into clean code that they had to be abandoned. I spent days, for instance, trying to get the AI to implement inner only grid borders in a clean way, the sort you see on a noughts and crosses board, where only the inside edges have lines. I never got it working the way I wanted, so eventually I let it go.

The overall result, however, was code that reads almost like English, and I am extremely proud of it. The lower level details that usually make components hard to read are abstracted behind clean, well named APIs. As the codebase grew this way, the AI got noticeably better at inferring the existing patterns.

It is worth noting that while I think this works well for vibe engineering a static site, I am not sure how it would translate to a large scale production application, where the designs are often handed to you and there are backend integrations to consider alongside the frontend.
...
- For anything that is a core part of your stack, it is still worth learning it from a reputable source rather than trusting AI blindly.
- AI is brilliant for design and for getting something on the screen fast, so prototype quickly and throw away what does not work.
- Start from a solid base, change one thing at a time, and commit the work you are happy with so the AI cannot overwrite it.
- Tests, linting, and TypeScript are what let the AI move fast and autonomously without breaking things.
- Decomposing into small, well named components leaves you with code that reads like English, which is easier for the AI to understand and reuse.
- Do not fight the tool. When something is genuinely hard for the AI, it is probably better to let it go than to force it.
- AI is a huge time saver for writing, but it still takes a lot of effort afterwards to make it sound like something an individual would actually say.
