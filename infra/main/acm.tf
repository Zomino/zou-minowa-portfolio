...
resource "aws_acm_certificate" "chat_api" {
  domain_name       = "api.${aws_route53_zone.site.name}"
  validation_method = "DNS"
...
resource "aws_acm_certificate_validation" "chat_api" {
  certificate_arn         = aws_acm_certificate.chat_api.arn
